...
impl<T> BasicInst<T> {
    ...
    fn src_mask(&self, imports: &[Import]) -> RegMask {
        match *self {
            ...
            BasicInst::Clz { src, .. } => RegMask::from(src),
            BasicInst::Andn { src1, src2, .. } => RegMask::from(src1) | RegMask::from(src2),
            BasicInst::Orn { src1, src2, .. } => RegMask::from(src1) | RegMask::from(src2),
    ...
    fn map_register(self, mut map: impl FnMut(Reg, OpKind) -> Reg) -> Option<Self> {
        ...
        match self {
            ...
            BasicInst::Clz { dst, src } => Some(BasicInst::Clz {
                src: map(src, OpKind::Read),
                dst: map(dst, OpKind::Write),
            }),
            BasicInst::Andn { dst, src1, src2 } => Some(BasicInst::Andn {
                src1: map(src1, OpKind::Read),
                src2: map(src2, OpKind::Read),
                dst: map(dst, OpKind::Write),
            }),
            BasicInst::Orn { dst, src1, src2 } => Some(BasicInst::Orn {
                src1: map(src1, OpKind::Read),
                src2: map(src2, OpKind::Read),
                dst: map(dst, OpKind::Write),
            }),
    ...
    fn map_target<U, E>(self, map: impl Fn(T) -> Result<U, E>) -> Result<BasicInst<U>, E> {
        Ok(match self {
            ...
            BasicInst::Clz { dst, src } => BasicInst::Clz { dst, src },
            BasicInst::Andn { dst, src1, src2 } => BasicInst::Andn { dst, src1, src2 },
            BasicInst::Orn { dst, src1, src2 } => BasicInst::Orn { dst, src1, src2 },
...
fn convert_instruction(
    ...
    match instruction {
        ...
        Inst::Clz { dst, src } => {
            ...
            emit(InstExt::Basic(BasicInst::Clz { dst, src }));
        ...
        Inst::Andn { dst, src1, src2 } => {
            ...
            let Some(src1) = cast_reg_non_zero(src1)? else {
                emit(InstExt::Basic(BasicInst::LoadImmediate { dst, imm: 0 }));
                return Ok(());
            };
            ...
            emit(InstExt::Basic(BasicInst::Andn { dst, src1, src2 }));
        ...
        Inst::Orn { dst, src1, src2 } => {
            ...
            let Some(src2) = cast_reg_non_zero(src2)? else {
                emit(InstExt::Basic(BasicInst::LoadImmediate { dst, imm: -1 }));
                return Ok(());
            };

            emit(InstExt::Basic(BasicInst::Orn { dst, src1, src2 }));
    ...
    for block_target in used_blocks {
        ...
        for (source, op) in &block.ops {
            let op = match *op {
                ...
                BasicInst::Clz { dst, src } => Instruction::clz(conv_reg(dst), conv_reg(src)),
                BasicInst::Andn { dst, src1, src2 } => Instruction::andn(conv_reg(dst), conv_reg(src1), conv_reg(src2)),
                BasicInst::Orn { dst, src1, src2 } => Instruction::orn(conv_reg(dst), conv_reg(src1), conv_reg(src2)),
